...
bam.files <- c(dir('/data/diana/DKS13_YNBChIPseq/OD1/', pattern = '*_sorted.bam$',full.names = TRUE),
               dir('/data/diana/DKS13_YNBChIPseq/OD5/', pattern = '*_sorted.bam$',full.names = TRUE))
...
t <- topTags(e,n= 6871)
t <- t$table
write.table(t,file='~/Projects/fixpeaks/C_OD1-edgeR.tsv',quote=FALSE,row.names = TRUE,
