...
# genes: headed tsv file with columns: gene start end seqname strand (where seqname is chromosome, and gene is an identifier)
# window: # bp upstream to designate as a promoter region
# bg: .bedgraph file with columns: chr start end score
genes=/data/daniela/Projects/fixpeaks/files/h99-cneoformans-noncoding-removed.gff
window=1000
# bgs=$(ls /data/diana/DKS13_YNBChIPseq/Bedgraphs/DKS13_*sorted_norm_50bp_smooth.bedgraph)
bgs=$1
...
for bg in ${bgs[@]}; do
	...
	for chr in ${chrs[@]}; do
		...
		wc -l ${sample}_final.bed
	done
done

# still have to remove the empty first line in each file 
